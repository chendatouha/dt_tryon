...
class CrossAttnDownBlock2DSpatialAttn(CrossAttnDownBlock2D):
    ...
    def forward(
            self, hidden_states, hidden_states_refs, temb=None, encoder_hidden_states=None, attention_mask=None,
            cross_attention_kwargs=None, seq_num=1
    ):
        ...
        for i, (resnet, attn) in enumerate(zip(self.resnets, self.attentions)):
            ...
            if self.training and self.gradient_checkpointing:
                ...
                def create_custom_forward(module, return_dict=None,seq_num=1):
                    def custom_forward(*inputs):
                        if return_dict is not None:
                            return module(*inputs, return_dict=return_dict, seq_num=seq_num)
                        else:
                ...
                hidden_states = torch.utils.checkpoint.checkpoint(
                    create_custom_forward(attn, return_dict=False, seq_num=seq_num),
                    hidden_states,
...
class Transformer2DModelSpatialAttn(Transformer2DModel):
    ...
    def forward(
        ...
        hidden_states_ref = hidden_states_ref.unsqueeze(1).tile([1, seq_num, 1, 1]).reshape(ref_batch*seq_num,
                                                                                              ref_h * ref_w, ref_dim)
        ...
        for block in self.transformer_blocks:
            ...
            hidden_states = block(
                ...
                hidden_states_shape=[batch//seq_num, seq_num, height, width, inner_dim]
            )
...
class BasicTransformerBlockSpatialAttn(BasicTransformerBlock):
    def __init__(
        ...
        # temporal attention
        self.norm_temporal = nn.LayerNorm(dim, elementwise_affine=norm_elementwise_affine)
    ...
    def forward(
        ...
        B, SEQ, H, W, C = hidden_states_shape
        # 1. Self-Attention
        if self.use_ada_layer_norm:
        ...
        # extract first half
        B, S, H, W, C = hidden_states_shape
        hidden_states = hidden_states[:, :H*W, :]

        # 2. Temporal attention
        # hidden_states [B*Seq, H*W, C]
        norm_hidden_states = self.norm_temporal(hidden_states)
        norm_hidden_states = norm_hidden_states.reshape([B, SEQ, H, W, C]).permute(0, 2, 3, 1, 4).reshape([B*H*W, SEQ, C])
        attn_output = self.attn_temporal(
            norm_hidden_states,
            encoder_hidden_states=encoder_hidden_states if self.only_cross_attention else None,
            attention_mask=attention_mask,
            **cross_attention_kwargs,
        )
        hidden_states = attn_output + norm_hidden_states
        # output shape [B*H*W, Seq, C]

        # 3. Feed-forward
        norm_hidden_states = self.norm3(hidden_states)
        ff_output = self.ff(norm_hidden_states)
        hidden_states = ff_output + hidden_states
        hidden_states = hidden_states.reshape([B, H, W, SEQ, C]).permute([0, 3, 1, 2, 4]).reshape([B*SEQ, H*W, C])
...
class UNetMidBlock2DCrossAttnSpatialAttn(nn.Module):
    ...
    def forward(
            ...
            cross_attention_kwargs=None, hidden_states_refs=None, seq_num=1
    ):
        res_outputs = {}
...
if __name__ == "__main__":
    pretrained_path = r'/ByteDance_sd2.1-base-zsnr-laionaes5'
    from unet_2d_condition_spatial_attn_v2 import UNet2DConditionModelSpatialAttn as Unet
    device = 'cuda:1'
    ref_net = Unet.from_pretrained(
        pretrained_path, subfolder='unet', in_channels=4,
    ...
    sample = torch.normal(0, 1, size=(2, 4, 32, 32)).to(device)
    sample2 = torch.normal(0, 1, size=(4, 4, 32, 32)).to(device)
    timestep = (torch.zeros([1], dtype=torch.float32) + 1.).to(device)
    _, result_refs = ref_net(sample, timestep, None)
    result2, _ = unet(sample2, timestep, None, hidden_states_refs=result_refs,seq_num=2)
    print(result2.shape)
    #
